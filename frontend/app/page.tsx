        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div key={i} className="ceramic-card animate-pulse p-8 rounded-3xl">
                <div className="bg-gradient-to-br from-amber-200 to-orange-200 h-72 rounded-3xl mb-8"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3 gap-12 px-8">
            {filteredProducts.map((product) => {
              const quantity = cartItems[product.name] || 0
              return (
                <Card
                  key={product.id}
                  className="ceramic-card p-8 rounded-3xl hover:scale-105 transition-all duration-500 max-w-2xl mx-auto w-full"
                >
